...
// Paths that should be excluded from service worker caching
  ...
  event.waitUntil(
    caches.open(STATIC_CACHE_NAME)
      .then((cache) => {
        ...
        return cache.addAll(STATIC_ASSETS.filter(url => !url.endsWith('.css')));
      })
      .then(() => {
...
function shouldExcludePath(urlPath) {
  return EXCLUDED_PATHS.some(path => urlPath.includes(path));
}
...
self.addEventListener('fetch', (event) => {
  ...
  // Skip excluded paths (auth pages, etc.)
  if (shouldExcludePath(url.pathname)) {
    console.log('Skipping service worker for excluded path:', url.pathname);
    return;
  }
        ...
        .then(response => {
          if (response.ok && response.status < 400) {
            ...
            caches.open(API_CACHE_NAME)
              .then(cache => {
                // Skip caching CSS files to fix styling issues
  ...
  // Only handle task pages with service worker
  if (!url.pathname.includes('/tasks')) {
    return; // Let browser handle non-task pages normally
  }
  
  // For task pages - Stale while revalidate with 404 handling
  event.respondWith(
    caches.match(event.request)
      .then(response => {
